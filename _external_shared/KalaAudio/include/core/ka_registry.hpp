...
namespace KalaAudio
{
	...
	//Stores local non-owning pointers per T instance inside the KalaAudioRegistry struct
	template<typename T>
		requires is_class_v<T>
	struct LIB_API KalaAudioHierarchy
	{
		T* thisObject{};
		T* parent{};
		vector<T*> children{};
	...
	struct LIB_API KalaAudioRegistry
	{
		...
		//KalaAudioHierarchy content for storing parent-child relations per instance of this class
		static inline unordered_map<T*, KalaAudioHierarchy<T>> hierarchy{};
			...
			unique_ptr<T> targetContent)
		{
			...
			//add hierarchy node
			hierarchy[raw] = KalaAudioHierarchy<T>{};
