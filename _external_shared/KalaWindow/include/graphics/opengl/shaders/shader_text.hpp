...
namespace KalaWindow::Graphics::OpenGL::Shader
{
	...
	R"(
		...
		void main()
		{
			...
			vec4 worldPos = uProjection * view * uModel * vec4(pos, 0.0, 1.0);
	...
	R"(
		...
		void main()
		{
			...
			bool useShadow = shadow;
			if (shadow && glow) useShadow = false;

			if (useShadow) base = ApplyShadow(base, safeColor, alpha);
			if (glow)      base = ApplyGlow(base, safeColor, alpha);
			if (underline) base = ApplyUnderline(base);
			if (striped)   base = ApplyStripes(base);
