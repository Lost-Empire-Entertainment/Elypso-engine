...
namespace GameFile
{
	void GameObjectFile::SaveGameObject(const shared_ptr<GameObject>& obj)
	{
		string gameobjectsFolder =
			path(Game::projectPath).parent_path().string()
			+ "\\scenes\\" + path(Game::scenePath).parent_path().stem().string() + "\\gameobjects";
	...
	void GameObjectFile::LoadGameObject(const map<string, string>& data, const string& folderPath)
	{
		...
		for (const auto& kvp : data)
		{
			...
			{
				string fullModelPath = Game::currentGameobjectsPath + "\\" + folderPath + "\\" + value;
			...
			else if (key == "textures")
			{
				...
				string fullTex0Path = split[0] == "DEFAULTDIFF"
					? "DEFAULTDIFF"
					: Game::currentGameobjectsPath + "\\" + folderPath + "\\" + split[0];

				if (fullTex0Path == "DEFAULTDIFF")
				{
					fullTex0Path = Game::filesPath + "\\textures\\diff_default.png";
					...
					&& !exists(fullTex0Path))
				{
					...
					textures.push_back(Game::filesPath + "\\textures\\diff_missing.png");
				}
				else textures.push_back(fullTex0Path);

				string fullTex1Path = split[1] == "DEFAULTSPEC"
					? "DEFAULTSPEC"
					: Game::currentGameobjectsPath + "\\" + folderPath + "\\" + split[1];
				if (fullTex1Path == "DEFAULTSPEC")
				{
					fullTex1Path = Game::filesPath + "\\textures\\spec_default.png";
				...
				string fullTex2Path = split[2] == "EMPTY"
					? "EMPTY"
					: Game::currentGameobjectsPath + "\\" + folderPath + "\\" + split[2];
				...
				string fullTex3Path = split[3] == "EMPTY"
					? "EMPTY"
					: Game::currentGameobjectsPath + "\\" + folderPath + "\\" + split[3];
			...
			else if (key == "shaders")
			{
				...
				string fullShader0Path = Game::filesPath + "\\shaders\\" + split[0];
				string fullShader1Path = Game::filesPath + "\\shaders\\" + split[1];
			...
			{
				string fullTexPath = Game::filesPath + "\\icons\\" + value;
